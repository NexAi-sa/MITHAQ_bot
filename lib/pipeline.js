...
function tryRequire(name) {
    try { return require(`./${name}`); } catch { return require(`./lib/${name}`); }
}
const ai = tryRequire('openai');
const sheets = tryRequire('sheets');
const twitter = tryRequire('twitter');
const telegram = tryRequire('telegram');
    ...
    try {
        ...
        for (const item of approved) {
            try {
                ...
                let mediaIds = null;
                try {
                    console.log(`🖼️ Generating and uploading image for row ${item.rowIndex}...`);
                    const b64Image = await ai.generateImage(tweets[0]);
                    const mediaId = await twitter.uploadMedia(b64Image);
                    mediaIds = [mediaId];
                } catch (imgError) {
                    console.error(`⚠️ Failed to generate/upload image for row ${item.rowIndex}:`, imgError.message);
                    // It will continue posting without image if it fails.
                }

                // 2. Post to X
                if (tweets.length > 1) {
                    await twitter.postThread(tweets, mediaIds);
                } else {
                    await twitter.postTweet(tweets[0], null, mediaIds);
                }
