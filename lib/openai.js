...
async function generateImage(tweetText) {
    ...
    // Simple, clean, flat icon style — small and lightweight
    const prompt = `Simple flat icon on a solid dark navy (#1a1a2e) background. ONE single minimal symbol or icon related to: ${tweetText.substring(0, 150)}. Style: single color (#c9a84c gold), no gradients, no shadows, no textures, no text, no words, no letters, no human faces or bodies. Think: a single emoji-like icon, extremely simple, clean lines, centered. Like a simple app icon.`;
